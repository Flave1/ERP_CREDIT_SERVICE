    ...
    public class WithdrawalController : Controller
    {
        ...
        public async Task<ActionResult<WithdrawalSetupRespObj>> GetAllWithdrawalSetupAsync()
        {
            try
            {
                ...
                var WithdrawalSetups = _mapper.Map<List<WithdrawalSetupObj>>(response);
                if(WithdrawalSetups.Count() > 0)
                {
                    foreach (var item in WithdrawalSetups)
                    {
                ...
                {
                    WithdrawalSetups = WithdrawalSetups,
